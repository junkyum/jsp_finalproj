...
function listAnswer(answer) {
	...
	var url="<%=cp%>/tboard/listReplyAnswer";
...
function countAnswer(answer) {
	var url="<%=cp%>/tboard/replyCountAnswer";
	$.post(url, {answer:answer}, function(data){
...
<c:if test="${sessionScope.member.userId!=vo.userId && sessionScope.member.userId!='admin'}">		   
        ...
        <div style="clear: both; min-height: 30px;">
            ...
            <div style="float: right; text-align: rigth;">
                <button type="button" class="btn btn-default btn-sm wbtn" onclick="sendLike('${vo.repleNum}', '1')">좋아요 (<span id="likeCount${vo.repleNum}">${vo.likeCount}</span>)</button>
            </div>                        
        </div>
